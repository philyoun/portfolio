...
하지만 만약 "`.`"이 any character을 매치시켜주는거라 할 때, 진짜 `.`을 매치하고 싶다면? <br />
escape을 사용하면 된다. <br />
...
<details>
<summary>14.3.1.1 Exercises sol</summary>
  1. 패스
<br />
<br />
  2. 
...
# str_view(x, "\"'\\\\") 이것도 되네
```
<img src="https://github.com/philyoun/portfolio/blob/master/docs/R%20for%20Data%20Science/14-Strings_files/14.3.1-5.png?raw=true" alt="14.3.1-5">
  <br />
  <br />
  3. (진짜 .) + any character + (진짜 .) + any character + (진짜 .) + any character 같은 정규표현식을 match가능. <br />
 <br />
...
<details>
  <summary>14.3.2.1 Exercises sol</summary>
1. 
...
str_view(x, "^\\$\\^\\$$")
```
<img src="https://github.com/philyoun/portfolio/blob/master/docs/R%20for%20Data%20Science/14-Strings_files/14.3.2-5.png?raw=true" alt="14.3.2-5">
<br />
  <br />
2.  <br />
1번.
...
#### 14.3.3.1 Exercises
 <br />
  <br />
<details>
  <summary>14.3.3.1 Exercises sol</summary>
</details>
 <br />
  <br />
...
#### 14.3.4.1 Exercises
 <br />
  <br />
<details>
  <summary>14.3.4.1 Exercises sol</summary>
</details>
 <br />
  <br />
   <br />
...
<details>
  <summary>14.3.5.1 Exercises sol</summary>
1. <br />
&emsp;&emsp;&emsp;1번. 똑같은 문자가 3번 연속으로 나와야함. 예를 들어, "aaa" <br />
&emsp;&emsp;&emsp;2번. 한 쌍의 문자들이, 상반된 순서로 나와야한다. 그러니깐, "abba" 같이. <br />
&emsp;&emsp;&emsp;3번. 두 개의 문자들이 연속으로 2번 나와야함. "a1a1" <br />
&emsp;&emsp;&emsp;4번. 원하는 문자 하나 + any character + 처음 그 문자 하나 + any character + 처음 그 문자 하나. <br />
&emsp;&emsp;&emsp;예를 들어, "abaca", "b8b.b" <br />
&emsp;&emsp;&emsp;5번. 이제 이걸 이해하면 다 이해했다고 볼 수 있다. <br />
&emsp;&emsp;&emsp;처음 (.)은 <code>\\1</code>로, 두 번째 (.)는 <code>\\2</code>로, 세 번째 (.)는 <code>\\3</code>으로 refer되는 거다. <br />
&emsp;&emsp;&emsp;그리고 가운데는 .*니깐 0 or more. <br />
&emsp;&emsp;&emsp;그래서 예를 들어보면, "abccba", "abc1cba" 혹은 "abcsgasgddsadgsdgcba" 다 되는 것. <br />
 <br />
2. <br />
&emsp;&emsp;&emsp;1번. <code>`^(.).*\1$`</code>라고 생각했는데, "a"라는 단어는 포함이 안 된다. <br /> 
&emsp;&emsp;&emsp;"a"도 조건에 만족하는 단어인데, 꼭 시작과 끝을 명시해놔서 안 잡히는듯. <br />
&emsp;&emsp;&emsp;정답은 <code>'^(.)((.*\1$)|\1?$)`</code> <br />

&emsp;&emsp;&emsp;2번. <code>`(..).*\1`</code>, 솔루션 페이지에는 <code>`"([A-Za-z][A-Za-z]).*\\1"`</code>라고 되어있음. 결과는 같음. <br />
&emsp;&emsp;&emsp;3번. <code>`(.).*\1.*\1`</code>
...
`str_detect()`의 다른 변형으로는 `str_count()`가 있다. <br />
...
`str_extract()`는 완전한 match를 준다. <br />
...
각각의 요소component들이 다른 수의 조각들을 갖고 있을 수 있기 때문에, list를 return한다. <br />
...
`fixed()`와 `regex()` 둘 다 `ignore_case`라는 인자argument를 갖는다. <br />
