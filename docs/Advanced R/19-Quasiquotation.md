...
들어가기 전에: <br />   quote한다는게 뭔지 알았음. 그래서 unquote를 할 때가 있었고, 이를 19.4에서 했음. <br />   근데 19.5는 Non-quoting임. unquote가 아닌 non-quoting은 또 뭔지?

base R은 quasiquotation을 implement한 하나의 함수가 있다. `bquote()` <br /> 얘는 unquoting하는데 `.()`를 사용한다.
...
`bquote()`라는 함수를, base R의 어떤 함수도 사용하지 않는다. <br /> 그리고 이 함수가, 어떻게 R 코드가 작성되는지에 별 영향을 안 주었다. <br /> `bquote()`를 효과적으로 사용할 수 없는, 3가지 문제가 있다.

1.  니 코드에만 쉽게 쓸 수 있다. 사용자user가 제공supply하는 임의의arbitrary 코드에는 적용하기 힘들다.

2.  리스트에 저장되어 있는 여러 개의 expression들을 unquote할 수 있도록 해주는, <br />   unquote-splice operator를 제공하지 않는다. 그러니까 rlang에서 `!!!`에 해당하는게 없다.

3.  environment와 동반된 코드를 handle하기에는 무리가 있다. <br /> 이건 특히 `subset()`과 같은, data frame의 문맥에서 코드를 evaluate하는 함수에 치명적이다.

인자argument를 quote하는 base 함수들은, indirect specification을 허용해주기 위해 다른 테크닉을 사용한다. <br /> base R은 unquoting을 사용하기보다, 선택적으로 quoting off를 사용하는데, <br />   그래서 나는 이걸 **non-quoting** 테크닉이라고 부른다.

<details> <summary>개인적인 정리</summary> 그러니까 결론은 unquoting을 안 쓴다는 것. <br /> rlang 패키지의 함수들은 quoting 함수들이지만, 선택적으로 unquote를 할 수 있어서, quasiquotation이라고 했는데, <br /> base R의 함수들은 리얼 quoting 함수들(unquote를 못하는) <br /> unquote를 할 수 있는 함수라곤 <code>bquote()</code>하나인데, <br />   위에 써진 문제점들이 있으니깐, unquoting 대신 quoting off를 할 수 있는 non-quoting 테크닉을 사용. </details> <br /> <br />

base R에서 볼 수 있는 4가지의 기본 형식들이 있다.

-   한 쌍의 quoting, non-quoting 함수들. <br /> 예를 들어, `$`는 2개의 arguments를 받는데, 2번째 argument는 quote된다. <br /> prefix form으로 쓰면 더 알아보기 쉽다: `mtcars$cyl` 을 \``` $`(mtcars, cyl) ``처럼 쓸 수 있음. <br /> 만약에 variable을 간접적으로 refer하고 싶다면, `[[`을 사용할 수도 있다. <br /> 이건 변수의 이름을 string으로 받아주기 때문.
...
`$`와 가깝게 연관있는 3개의 quoting 함수들이 있다. `subset()`, `transform()`, `with()` <br /> 이것들은 `$`의 wrapper들로 간주되며, interactive한 사용에만 적합한 것. <br />   그래서 모두가 `[`를 사용한 non-quoting 대안이 있다. <br /> `<-`, `assign()`은 `$`와 비슷하게, `::`, `getExportedValue()`는 `[`와 비슷하게 작동한다.

-   quoting과 non-quoting 인자들arguments라는 하나의 쌍. <br /> 예를 들어, `rm()`은 생 변수 이름을 `...`에다가 공급할 수 있게 해주거나,이게 non-quoting <br />   list에다가 변수 이름들의 character vector를 공급할 수 있게 해준다. 이게 quoting
...
-   다른 인자argument가 quoting인지 non-quoting인지를 컨트롤하는 인자argument. <br /> 예를 들어 `library()`에서, <br />   `character.only`라는 인자argument는, <br />   첫 인자argument인 package의, quoting behavior를 컨트롤한다.
...
-   Evaluation이 실패하면 quoting. <br /> 예를 들어서, `help()`의 첫 번째 arguent를 string으로 evaluate하면 non-quoting이다. <br /> evaluation이 실패하면, 첫 번째 argument는 quoted. <br /> 그러니깐, 먼저 evaluate를 해보고, 안 되면 quote를 하는 것.
...
quoting 함수들의 중요한 다른 class는, base modelling과 plotting functions. <br />   소위 standard non-standard evaluation 룰들을 따르는 것들. <br /> <http://developer.r-project.org/nonstandard-eval.pdf>

예를 들어, `lm()`은 `weight`이랑 `subset` 인자들을 quote한다. <br /> 그리고 formula 인자와 사용되었을 땐, plotting 함수는 aesthetic 인자(`col`, `cex` 등)들을 quote한다. <br />   plotting 함수는 aesthetic arguments를 quote한다.

그래서 다음의 예와 같이, `col = iris$Species` 대신에 `col = Species`만 써도 된다.
...
이 함수들은 indirect specification을 할 수 있는, 만들어져 있는built-in 옵션들이 없지만, <br />   unquoting을 시뮬레이트할 수 있는 방법을, [Section 20.6]()에서 배울 것이다.
...
`!!!`은 유용하다. 왜냐하면 호출call에 넣고 싶은 것이 expression의 리스트인 경우가 흔하기 때문이다. <br /> 사실, 이런 패턴이 다른 곳elsewhere에서도 흔하다. <br /> 다음 2개의 motivating problems를 보아라.

-   `...`에 넣고 싶은 elements가, 이미 리스트에 저장되어 있는 경우에 어떻게 하겠는가? <br /> 예를 들어, 데이터 프레임들의 리스트가 있고, `rbind()`를 하고 싶다고 해보자.
...
이 특정한 예에서는 `rbind(dfs$a, dfs$b)`를 하겠지만, 임의의 길이에 대해서는 어떻게 일반화할건지?

-   argument의 이름을 간접적으로 supply하고 싶다면? <br /> 예를 들면, single column 데이터 프레임을 만들고 싶다. 칼럼 이름은 variable에 저장되어있고.
...
생각할 수 있는 한 가지 방법은 quasiquotation과 명백하게 parallel한 걸 그려보는 것. <br /> One way to think about these problems is to draw explicit parallels to quasiquotation.

1번 문제. <br /> 여러 개의 데이터 프레임들을 row-binding하는 것은, unquote-splicing하는 것과 같다. <br /> 리스트 안의 개별적인 원소들individual elements을 호출call 안으로 inline하고 싶다.
...
`!!!` 없이 그냥 해야한다. <br /> Hadley가 묘사하고 싶은 것은, 파이썬에서 임의의 개수의 원소들을 받게 될 때 사용하는 `*args`, `**kwarg` 와 같은, <br /> argument unpacking이라고 부르는 것과 연관된 걸 설명하려나본데, 이 케이스는 아니다. <br /> Ruby, Go, PHP, Julia 같은 프로그램에선 spatting이라고 하나보다.

`...`로 넘겨주고 싶은 elements가, 이미 리스트에 저장되어 있는 경우에 `!!!`를 이용해서 unquoting slice하려는 걸 알겠는데, <br /> 잘못된 케이스를 보여준 것 같다.

2번 문제. <br /> `=`의 왼쪽을 unquoting하는 것과 같다. <br /> `var`을 interpreting하지말고, `var` 변수에 저장되어 있는 값들을 쓰고 싶음.
...
`:=`는 마치 퇴화된 기관vestigial organ과 같다. <br /> R의 parser에 의해 인지recognised는되는데, 이것과 연관되어있는 코드는 없다. <br /> `=`와 비슷하게 보이지만, 반대 쪽에 expression이 올 수 있도록 허용해주고, <br /> `=`보다 좀 더 유연한 대안flexible alternative이 된다.

base R은 다른 접근을 취하는데, Section 19.6.4에서 다룰 것이다. <br /> 얘는 좀 신기하게 하던데. `do.call()`과 몇몇 도움으로 모든 걸 해냄.

이렇게 quoting arguments없이 이러한 툴들을 support해주는 함수들을, tidy dots를 갖고 있다고 한다. <br /> We say that functions that support these tools, without quoting arguments, have tidy dots.

니가 만든 함수가 tidy dots 행동behavior을 갖도록 하려면, `list2()`를 이용하면 된다. <br /> To gain tidy dots behavior in your own function, all you need to do is use list2(). <br /> 다음 Section에서 바로 하게 된다.

### 19.6.1 Examples

`list2()`를 이용할 수 있는 하나의 예는, <br />   attributes를 유연하게 설정하게끔, `attributes()`에다가 wrapper을 씌워줄때.
...
tidy dots가 없는 함수들에 대해서도 이러한 테크닉을 쓰고 싶다면? <br /> 하나의 옵션은 `rlang::exec()`을 써서, <br />   some arguments를 `...`로 직접적으로 받고, 나머지는 리스트로 간접적으로indirect 받는 그런 함수를 호출하는 것.
...
## [1] 0.3671933 0.3249865 0.2526103
```

`exec()`은 `call2()`와 밀접하게 연관되어 있다. <br /> `call2()`는 expression을 return하고, `exec()`는 evaluate를 한다.

### 19.6.3 `dots_list()`

`list2()`는 다른 편리한 특징: 디폴트로, 마지막에 비어있는 인자들arguments을 다 무시한다. <br /> `tibble::tibble()`과 같은 함수에서는 이러한 특성이 유용한데, <br />   왜냐면 변수 순서를 맘대로 바꿀 수 있다는 뜻이기 때문. <br /> 무슨 말인지 이해 안 될수도 있는데, 그냥 다음의 예를 보면 된다.
...
위 2개의 차이란? <br /> tibble은 마지막이 ,로 끝나도 상관이 없다는 것. 그래서 y, z를 그냥 그대로 잘라내서 뒤에 붙여도 작동 <br /> 그에 비해 data.frame은 마지막 줄에 ,를 추가하고, z 줄에 ,를 없애야 작동.

`list2()`는, `rlang::dots_list()`의 wrapper다. <br /> `dots_list()`의 가장 흔한 것들을, 디폴트로 셋해놓은 게 `list2()`. <br /> 아예 `dots_list()`를 직접 호출함으로써, 더 많은 컨트롤을 얻을 수 있다.

-   `.ignore_empty`를 쓰면, 정확하게 어떤 인자들arguments을 무시할지 컨트롤할 수 있다. <br /> 디폴트는 끝에 나오는 하나의 인자single trailing argument를 무시하는거고, <br />   모든 missing arguments를 무시할 수도, 아니면 다 무시하지 않을 수도 있다. <br /> 위의 tibble에서 ,로 끝나도 상관 없는 것은, `.ignore_empty`가 디폴트로 'trailing'이기 때문.

-   `.homonyms`는 만약 여러 개의 인자들이 같은 이름을 쓸 때, 어떻게 할지를 컨트롤한다. <br /> `dots_list(x = 1, x = 2)` 같은 걸 어떻게 할 건지?

-   무시되지 않은 empty 인자들이 있을 때, `.preserve_empty`는 어떻게 할지를 컨트롤한다. <br /> 디폴트는 에러를 반환하는거고, `.preserve_empty = TRUE`라고 하면 missing symbols를 반환한다. <br /> `dots_list()`로 함수 호출을 만들 때 이용하는거라면 유용.
...
base R은 이 여러가지 문제들을 하나의 도구로 해결한다. : `do.call()` <br /> `do.call()`은 2개의 주요한 인자들main arguments을 받는데, <br />   하나는 what, 어떤 함수를 호출할지. <br />   다른 하나는 args, 그 함수에게 패스해줄 list of arguments <br /> 그래서 `do.call("f", list(x, y, z))`라고 한다면, `f(x, y, z)`라는 뜻이다.

1.  `do.call()`은 위에 1번 문제를 깔끔하게 풀어준다. <br /> 여러 개의 데이터 프레임들을 rbind해줌.
...
1.  좀 더 발전을 시키면, 2번 문제도 `do.call()`을 사용해서 해결할 수 있다. <br /> 먼저 list of arguments를 만들고, 그 리스트에 이름을 붙여준 다음, `do.call()`을 사용하면 된다.
...
몇 가지 base 함수들을 사용하면, `do.call()` 사용하는 걸 피할 수 있다. <br /> (예를 들면, `interaction()`, `expand.grid()`, `options()`, `par()` 같은 것들)

만약에 `...`의 첫 번째 요소component가 리스트라면, <br />   `...`의 다른 원소들elements을 보기보다는 리스트의 components를 받는다. <br /> implementation은 다음과 같은 식으로 간다.
...
`do.call()`을 피하는 다른 방법을 `RCurl::getURL()`에서 찾았다.(이 함수는 Duncan Temple Lang이 작성) <br /> `getURL()`은 `...`이랑 `.dots`를 받고 같이 묶는다. <br /> 그래서 다음과 같이 생긴다.
...
hadley가 이걸 보고 감명받아서 tidyverse에 많이 썼는데, 지금은 이전의 방법을 선호한다.
...
quasiquotation에 대한 아이디어를 단단하게 만들기 위해, <br />   이 섹션에서는 real problems을 해결하는 몇 가지 케이스들을 모아놓았다. <br /> 몇몇 케이스에서는 purrr도 썼다. <br /> quasiquotation과 functional programming의 조합combination이 특히나 우아하더라.elegant

### 19.7.1 `lobstr::ast()`

`lobstr::ast()`에 있어 일어나는 번거로운 일들을 quasiquotation은 해결해준다. <br /> 이미 캡쳐된 expression을 `ast()`해보면 어떻게 될까?
...
`ast()`는 첫 번째 인자를 quote하기 때문에, `!!`로 unquoting을 해줘야 한다. <br /> unquoting을 해주면 해결이 된다.
...
quasiquotation은 코드를 만드는데generating code 있어 강력한 툴이 되어준다. <br /> 특히나 `purrr::map()`이나 `purrr::reduce()`와 결합combine되어 사용될 때 강력. <br /> 예를 들어, coefficients가 정해진 linear model을 가지고 있다고 생각해보자.
...
목표: 이걸 `10 + (x1 * 5) + (x2 * -4)` 라는 expression으로 바꾸고 싶음.

가장 먼저 해야할 것은 '캐릭터 names 벡터'를 'symbols 리스트'로 바꾸는 것. <br /> `rlang::syms()`가 딱 이런 상황을 위해 디자인되었다. <br />
...
`(x1 * 5)`, `(x2 * -4)` 이것들을 만들어야 되는데, <br /> 그리고 이 각 변수 이름variable name과 이 계수를 combine해야 한다. <br /> `rlang::expr()`, `purrr::map2()`를 이용해서 할 수 있다. <br />   `map2()`는 `map()`의 다변수 버전.
...
이제 여기다 intercept도 포함을 시켜주자. <br /> 그냥 summands 벡터의 첫 번째에 추가해주면 됨.
...
`reduce()`는 각 terms를 single sum으로 만들어주는 역할([Section 9.5](https://blog-for-phil.readthedocs.io/en/latest/Advanced%20R/09-Functionals/#95-reduce-family))
...
이걸 좀 더 일반적general이게 만들어보자. <br /> 사용자가 계수 이름을 제공supply하게끔 하고, 여러가지 변수 이름 쓰지말고(`x1`, `x2`, ...) 그냥 변수 하나(`x`)에다가 인덱스로. <br /> 예를 들어서, `x, c(10, 5, -4)`라고 사용자가 제공supply하면, 결과물이 <br />   `10 + (5 * x[[1L]]) + (-4 * x[[2L]])` 이렇게 나오도록 해보자.
...
이러면 됐고, 이걸 wrapper로 씌워서, 함수로 만들어 좀 더 깔끔하게.

``` r
linear <- function(var, val) {
    var <- ensym(var)
    coef_name <- map(seq_along(val[-1]), ~expr((!!var)[[!!.x]])) # val[-1]의 개수만큼 1L, 2L,... 만듬

    # (5 * x[[1L]]), (-4 * x[[2L]])의 꼴들을 만들어야
...
`ensym()`을 사용한 것을 인지할 것: 사용자로 하여금, 한 개 변수의 이름을 공급하길 원한다.   복잡한 expression이 아니라.

### 19.7.3 Slicing an array

차원dimension과 인덱스index가 주어졌을 때, array에서 해당 부분을 추출하고 싶다치자. <br /> `slice(x, 1, 3)`이라고 하면, first dimension에서 third slice, 즉, `x[3, , ]` <br /> `slice(x, 3, 1)`이라고 하면, third dimension에서 first slice, 즉, `x[, , 1]` <br /> `slice(x, 2, 3)`이라고 하면, second dimension에서 third slice, 즉, `x[, 3, ]` <br /> missing 인자들arguments을 다뤄야 하기 때문에 좀 어렵다.

여러 개의 missing arguments를 다루는 호출call을 생성해야 한다. <br /> 먼저, `rep()`와 `missing_arg()`를 이용해, missing arguments의 리스트를 만들어야 한다. <br /> 그리고나서 call로 unquote해줘야한다.
...
그리고 subset 할당을 이용해 원하는 곳에다 index를 넣어주면 됨.
...
이걸 `stopifnot()`들을 넣어서, 인터페이스를 깔끔하게 만들고, 함수로 wrap해보자.
...
slice <- function(x, along, index) {
    ...
    # stopifnot(length(along) == 1, length(index) == 1) # 이렇게 써도 되는거아닌가?
...
그리고 이걸 `eval()`을 이용해서 값을 진짜로 볼 수도 있음.
...
quotation의 강력한 응용 중 하나는, 함수를 손으로by hand 만들 수 있다는 것. <br /> `rlang::new_function()`을 사용해서. <br /> 이 함수는, <br />   arguments, body, env(얘는 옵션)라는 3개의 요소들components을 받아 함수를 생성하는 그런 함수. <br /> ([Section 6.2.1](https://blog-for-phil.readthedocs.io/en/latest/Advanced%20R/06-Functions/#621-function-components))
...
주의: `exprs()`에 아무런 인자들이 없으면, 디폴트 없이 인자들을 만들어준다.

`new_function()`을 쓰는 한 예로는, scalar 혹은 symbol arguments를 쓰는 function factories의 대체제. <br /> 예를 들어, 함수를 숫자만큼 제곱하는 함수를, 생성하는 함수를 작성할 수 있다. <br /> we could write a function that generates functions / that raise a function to the power of a number.
...
`new_function()`의 또 다른 응용으로, `graphics::curve()`와 같은 것도 만들 수 있다. <br /> 함수를 만들지 않고서, 수학적 표현을 그릴 수 있도록 해주는 것임.

``` r
curve(sin(exp(4 * x)), n = 1000)
```

![](19-Quasiquotation_files/figure-markdown_github/unnamed-chunk-82-1.png)

이 코드에서, `x`는 대명사pronoun다. <br /> `x`는 하나의 값으로 딱 정해진게 아니라, plot에서 range별로 변하는 그런 placeholder이다. <br /> it doesn't represent a single concrete value, but is instead a placeholder <br />   that varies over the range of the plot.

`curve()`를 implement하는 하나의 방법은, <br />   해당 표현식을 하나의 인자 `x`를 가진 함수로 바꾸고, 해당 함수를 호출하는 것.

``` r
curve2 <- function(expr, xlim = c(0, 1), n = 100) {
  expr <- enexpr(expr)
  f <- new_function(expr(x = ), expr)
...
`curve()`와 같이, 대명사를 가진 expression을 사용하는 함수들을, **anaphoric** functions라고 한다.
...
quasiquotation이라는 아이디어는 오래되었다. <br /> 1940년대 초기, 철학자 Wilard van Orman Quine에 의해 처음 개발되었다. <br /> 철학에서 먼저 필요했던 이유는, 단어를 언급하는 것과 사용하는 것간의 구분을 지어야했기 때문.

quasiquotation은, 1970년대 중반 Lisp라는 프로그래밍 언어에서 처음으로 사용되었다.(Bawden 1999) <br /> Lisp는 quoting 함수로 `` 을, unquoting하는데 `,`을 사용한다. <br /> Lisp의 유산인 대부분의 언어들은 비슷하게 작동한다. <br /> 예를 들어, Racket은 ``와 `@`를, Clojure은 \`\`\`\``와`~`를, Julia는`;`와`@\`를 사용. <br /> 이 언어들은 하나의 quoting 함수가 있고, 이걸 명백하게 호출해야 한다.

하지만 R에서는, 많은 함수들이 하나 이상의 인풋들을 quote한다. <br /> 이건 애매모호함ambiguity를 일으키지만(인자argument가 quote된건지 아닌건지를 다큐먼트 읽어보고 알아내야하기 때문), <br />   간결하고 우아한 데이터 탐구 코드를 허락한다. <br />   allows for concise and elegant data exploration code. <br /> base R에서는, 딱 하나의 함수인 `bquote()`만이 quasiquotation을 지원한다. <br /> 이건 2003년 Thomas Lumley에 의해 작성된 것. <br /> 하지만, 위에서 말한 `bquote()`는 주요한 한계들이 있어서 R 코드에 넓게 적용되진 못함.

이 한계들을 해결하려는 Hadley의 노력으로, lazyeval 패키지를 만들었다. <br /> 하지만 문제들 분석이 불완전했고, lazyeval이 몇몇 문제들을 해결하긴 했는데, 다른 문제들을 만들었음. <br /> Lionel Henry와 함께 작업을 해서, tidy evaluation 프레임워크(2017)로 모든 걸 제대로 해결함. <br /> tidy evaluation의 새로움에도 불구하고, 풍부하고 강력한 이론이기 때문에, <br />   한 번 마스터하고 나면 많은 어려운 문제들을 훨씬 쉽게 해결할 수 있어서, 여기서 다루었다.
